        ...
        <div className={`output-panel ${outputCollapsed ? 'collapsed' : ''}`}>
            <div className="output-header" onClick={() => setOutputCollapsed(!outputCollapsed)}>
                ...
                <div className="output-actions" onClick={(e) => e.stopPropagation()}>
                    <button className="output-action-btn" title="Clear" onClick={clearOutput}>
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                            <polyline points="3 6 5 6 21 6" />
                            <path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6" />
                        </svg>
                    </button>
                    <button className="output-action-btn" title={outputCollapsed ? 'Expand' : 'Collapse'} onClick={() => setOutputCollapsed(!outputCollapsed)}>
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                            {outputCollapsed
            ...
            {!outputCollapsed && (
                <div className="output-body" ref={bodyRef}>
                    {outputLines.length === 0
                        ? <span className="output-placeholder">Ready. Press F5 to compile.</span>
